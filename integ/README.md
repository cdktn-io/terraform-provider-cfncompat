...
source, run real `terraform init`/`apply`/`destroy` cycles, and (for
`TestE2ECustomResource`) deploy real AWS resources. Run them on demand, locally
...
- AWS credentials with permission to create S3 buckets, IAM roles, and Lambda
  functions — only required for `TestE2ECustomResource` / `make custom-resource`.
  `TestE2EFunctions` / `make functions` needs no AWS credentials at all.
...
The Makefile exposes this as pattern targets on top of `functions` and
`custom-resource`:
...
(likewise `custom-resource-no-cleanup`, `custom-resource-validate-only`,
`custom-resource-cleanup-only`).
...
- `custom-resource-e2e` only runs when the `run_aws` workflow input is `true`,
  and needs a `AWS_E2E_ROLE_ARN` repository variable (an IAM role trusting
  this repo's GitHub OIDC provider) to assume via
