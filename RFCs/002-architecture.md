...
| **`TerraformIntrinsicResolver`** | `IIntrinsicResolver` (I2) | Maps the verified table: `Fn::Join`→`join()`/`${a}${b}`, `Split`→`split`, `Select`→`element`, `Base64`→`base64encode`, `ToJsonString`→`jsonencode`, `Length`→`length`, `Sub`→HCL template, `FindInMap`→`m[k1][k2]` when synth-known. `resolvePseudo`: `AWS::Region`→`${data.aws_region.current.name}`, `AWS::Partition`→literal when `ENABLE_PARTITION_LITERALS` (`stack.ts:792-796`) else `${data.aws_partition.current.partition}`. `requireCapability('AWS::LanguageExtensions',…)` is a **no-op** (suppresses `addTransform`, `cfn-fn.ts:538/949/978`); token-valued `Fn::Cidr` and the custom-resource protocol register the **polyfill provider**; `Fn::Transform` throws a clear synth-time error. **Recursive key mapping (cdktn-planning#1):** applies the generated per-resource CFN-name→terraform-attribute key map to any CFN property-bag object literal embedded in an intrinsic's arguments — not just a resource's own `Properties` — e.g. aws-cdk-lib's `Fn.conditionIf(cond, Aws.NO_VALUE, { EncryptionType: 'KMS', KeyId: '…' })` (`aws-kinesis/lib/stream.ts:1005-1007`) must reach `provider::cfncompat::condition_if` with `encryption_type`/`key_id` keys or the awscc schema rejects it. The map is a **shared export emitted by `@cdktn/awscc` codegen** (the same artifact `AwsccL1Renderer` uses for the top-level `Properties` walk), never renderer-internal state, and it must be *generated*, not computed: awscc's snake_case is not mechanically derivable from the CFN name (`Ipv4IpamPoolId`→`ipv_4_ipam_pool_id`, not `ipv4_ipam_pool_id`; same for `ipv_6_*`, `LoRaWAN`, `ARNs`). Note the direction: the map is keyed by the **CFN PascalCase spelling** — the exported camelCase-keyed `ccXxxPropertyToTerraform` mappers alone don't cover an inline PascalCase literal, and PascalCase→camelCase is itself not "lowercase the first letter". |
...
3. **`cfnPropertyNames` dict status.** Declared at `cfn-resource.ts:93` (per `core/adr/box-api.md:133-143`) but not populated — WIP or deferred? Natural codegen home for a dual snake_case key map.
   **Resolved (2026-08-31): not depended upon.** We emit our own per-resource CFN-PascalCase→snake_case key map from `@cdktn/awscc` codegen (cdktn-planning#1; consumed by `AwsccL1Renderer` and, recursively, `TerraformIntrinsicResolver` — see the §3 row). The map must be generated rather than computed (`Ipv4IpamPoolId`→`ipv_4_ipam_pool_id`), and `@cdktn/awscc` owns the authoritative pairs already. The question to the AWS CDK team stays open as informational only; a populated `cfnPropertyNames` would be a convenience, not a dependency.
