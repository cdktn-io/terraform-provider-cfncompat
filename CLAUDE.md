...
- `internal/provider/function_*.go`, `resource_*.go`, `data_source_*.go` — one file per provider-defined function, resource, and data source, each with a `*_test.go` beside it, and each registered in the matching `provider.go` method. Anything that talks to AWS takes its client as a **narrow interface** declared in the same file (`callerIdentityGetter`, `availabilityZonesAPI`, `ssmParameterGetter`, the custom-resource engine's Lambda/SNS/S3 interfaces) so the unit tests drive the logic with fakes and need no AWS.
- Shared, non-registered helper files sit beside them: `region_facts.go` (offline partition / URL-suffix table), `ssm_parameter_read.go` (the GetParameter call, CloudFormation-shaped error mapping, hand-rolled validators and `Configure` unwrapping shared by the four dynamic-reference data sources), `ssm_value_types.go` (the `AWS::SSM::Parameter::Value<...>` inner-type table and its AWS-side existence checks), and `dynamic_reference.go` (the `{{resolve:...}}` grammar shared by the two dynamic-reference functions).
...
- `terraform-plugin-framework-validators` is deliberately not a dependency: schema validators are hand-rolled (`pseudoParametersStackNameValidator`, `stringNotEmptyValidator`, `stringOneOfValidator`, `int64AtLeastValidator`).
